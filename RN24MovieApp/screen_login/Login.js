...
export default class Login extends Component{
        ...
        return (
            <View style={style.root}>
                ...
                <View style={style.content}>
                    {/* 1.1 로고 글씨 */}
                    <Text style={style.logo}>Introduce Movie</Text>
                    {/* 1.2 이메일/패스워드 입력박스 */}
                    {/* TextInput은 로그인, 회원가입, 비밀번호 리셋 화면에 모두 사용되므로 사용빈도가 높음 */}
                    {/* 이를 일일이 스타일링하는 것이 번거롭고 수정도 어려워서, 별도의 컴포넌트로 제작하여 재사용함 */}
                    ...
                    <Text style={style.resetPW} onPress={()=>{this.props.navigation.navigate('ResetPW')}}>비밀번호 변경</Text>
                    {/* 1.4 로그인 버튼 */}
                    {/* 가로 전체 사이즈의 버튼을 만들기 위해 View로 깜싸기 */}
    ...
    login = async ()=>{
        // 원래는 서버에 전송하는 코드 사용해야 하지만 시간상 생략
