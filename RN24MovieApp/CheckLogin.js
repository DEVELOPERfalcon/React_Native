...
const CheckLogin = (props)=>{  // stateless는 props를 파라미터로 전달받음, stateful은 없음

    // AsyncStorage에서 로그인 유무 읽어오기
    AsyncStorage.getItem('isLogin')
    .then((value)=>{
        if(value) props.navigation.navigate('drawerNav'); // stateful은 this.Props이고 stateless는 전달받은 props
...
export default CheckLogin;  // stateless는 export default를 따로 해야 한다
